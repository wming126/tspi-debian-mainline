# RK3566 TaiShan Pi (泰山派) Debian Mainline OS

[![Kernel](https://img.shields.io/badge/Kernel-Mainline-blue.svg)](https://kernel.org)
[![OS](https://img.shields.io/badge/OS-Debian%2012%20Bookworm-orange.svg)](https://www.debian.org)
[![Hardware](https://img.shields.io/badge/Hardware-RK3566-red.svg)](https://www.rock-chips.com/)

这是一个专门为 **RK3566 泰山派 (TaiShan Pi)** 打造的自动化构建系统。它能够生成一个纯净、高性能、且完美适配硬件的 **Debian 12 (Bookworm)** 操作系统，采用最新的 **主线 Linux 内核 (Mainline Kernel)**以及**最新的主线u-boot**。

---

## 🌟 项目亮点

*   **纯净主线内核**：告别原厂老旧内核，拥抱最新的 Linux 社区特性。
*   **全硬件加速**：
    *   **GPU (Mali-G52)**：通过 Panfrost 开源驱动实现 OpenGL 硬件加速。
    *   **HDMI 显示**：完美支持 1080P/4K 输出。
*   **无线功能完备**：
    *   **WiFi (AP6212)**：解决 `clm_blob` 缺失导致的扫描问题。
    *   **Bluetooth**：修复了 3Mbps 下的复位超时问题，稳定工作在 1.5Mbps（**相关补丁已尝试提交上游**）。
*   **轻量级桌面**：预装 **XFCE4** 桌面环境，启动即用。
*   **自动化加固**：
    *   **智能路由**：WiFi 优先级高于 USB 虚拟网卡，避免断网。
    *   **时间同步**：预设阿里云/腾讯云 NTP 服务器，解决嵌入式无电池时间重置问题。
    *   **自动扩容**：首次开机自动扩展 Root 分区至 eMMC 全容量。

---

## 🛠️ 构建指南

### 1. 克隆代码与初始化子模块
由于本项目引用了主线内核与 U-Boot 源码作为子模块，克隆后需要执行初始化：
```bash
# 克隆仓库
git clone https://github.com/your-username/your-repo.git
cd your-repo

# 初始化并更新子模块 (Linux & U-Boot)
git submodule update --init --recursive
```

### 2. 准备环境 (Ubuntu/Debian)
安装构建所需的依赖包：
```bash
sudo apt update
sudo apt install -y \
    build-essential gcc-aarch64-linux-gnu \
    bc bison flex libssl-dev device-tree-compiler \
    debootstrap qemu-user-static \
    gdisk e2fsprogs uuid-runtime \
    git-email parted
```

### 2. 构建 RootFS
从零下载 Debian 基础包并配置环境：
```bash
sudo ./build_rootfs_from_scratch.sh
```

### 3. 生成完整镜像
编译 U-Boot、内核并组装成一个 `.img` 文件：
```bash
sudo ./build_complete_image_v4.sh
```

生成的镜像位于：`output/tspi_unified_image_v4.img`

---

## 🚀 烧录与使用

### 烧录方法 (Maskrom 模式)
使用 `rkdeveloptool` 或 `upgrade_tool` 进行烧录：
```bash
sudo ./rkdeveloptool/rkdeveloptool wl 0 output/tspi_unified_image_v4.img
```

### 默认账户
*   **用户名**: `root`
*   **密码**: `root`

---

## 📈 上游贡献 (Upstreaming)

本项目致力于将修复反馈给主线内核社区。
*   **蓝牙稳定性补丁**：更新了 `compatible` 字符并优化了 UART 波特率，解决了 `Reset failed (-110)` 错误。

---

## 📂 目录结构

*   `src/`：存放内核、U-Boot 和 rkbin 源码。
*   `firmware/`：必要的无线网卡固件补丁。
*   `tools/`：调试与辅助工具。
*   `doc/`：详细的开发与调试日志。

---

## 📜 开源协议

本项目采用 **MIT License**。内核与 U-Boot 遵循其各自的 GPL 协议。

---

**由 Ming Wang (wangming5719@gmail.com) 维护。**
如果你觉得这个项目对你有帮助，欢迎点一个 **Star**! ⭐
